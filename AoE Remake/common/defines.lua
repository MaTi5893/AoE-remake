...
country = {
    ...
    UNCIV_TECH_SPREAD_MAX = 1,
    UNCIV_TECH_SPREAD_MIN = 1,
...
military = {
    ...
    POP_MIN_SIZE_FOR_REGIMENT_COLONY_MULTIPLIER = 7,
    POP_MIN_SIZE_FOR_REGIMENT_NONCORE_MULTIPLIER = 6, -- VALUE * POP_MIN_SIZE_FOR_REGIMENT is min for noncores
    ...
    LEADER_PRESTIGE_TO_MORALE_FACTOR = 0.8, -- f.ex. 100% of prestige = +20% morale
    LEADER_PRESTIGE_TO_MAX_ORG_FACTOR = 0.4, -- f.ex. 100% of prestige = +20 max org
    ...
    AI_ESCORT_RATIO = 4.0, -- ratio of escorts to transports in invasion fleets
    ...
    RECON_UNIT_RATIO = 0.3, -- the % of units in the army that must have a recon value to get the full bonus
    ENGINEER_UNIT_RATIO = 0.3, -- the % of units in the army that must have a fort attack value to get the full bonus
    SIEGE_BRIGADES_MIN = 5, -- the number of brigades needed for a siege to progress at normal speed
    SIEGE_BRIGADES_MAX = 10, -- the number of brigades above which you get no addition benefit in sieges
    SIEGE_BRIGADES_BONUS = 0.75, -- the bonus to siege speed from each brigade
...
diplomacy = {
    ...
    GW_WARGOAL_JINGOISM_REQUIREMENT_MOD = 0.00, -- % deduction of required jingoists in country
    GW_WARSCORE_COST_MOD = 0.75, -- cost reduction factor on warscore needed to fulfill goal if great war
    GW_WARSCORE_COST_MOD_2 = 0.65, -- cost reduction factor on warscore needed to fulfill goal if late game great war
    ...
    CRISIS_BASE_CHANCE = 0.1, -- removed for AoE
...
ai =
{
    ...
    SPAM_PENALTY = 25, -- makes certain diplomatic action less common
    ...
    NAVAL_SUPPLY_PENALTY_LIMIT = 0.3, -- AI will allow to have max X% of supply penalty (when too little naval bases)
    ...
    PEACE_FORCE_BALANCE_FACTOR = 0.4, -- AI willingness to peace based on strength estimation of both sides
    ...
    MAX_FLEETS = 12, -- Maximal amount of main fleets the AI will divide its navy into (does not include specialized fleets such as blockades and naval invasions)
